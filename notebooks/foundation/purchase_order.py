...
# MAGIC %md
# MAGIC #### Find the last processed ingestion timestamp
...
last_processed_timestamp = (
    ...
    .agg(
        F.max("source_updated_timestamp")
        .alias("max_timestamp")
    )
    .first()["max_timestamp"]
)
...
standardized_df = (
    incremental_df
    .withColumn(
        "source_record_id", F.concat_ws(
            "|", F.col("purchase_order_id"), F.col("purchase_order_line"),
        ),
    )
    .withColumn(
        "purchase_order_id",
        F.upper(
            F.trim("purchase_order_id")
        ),
    )
    .withColumn("purchase_order_line", F.trim("purchase_order_line"),)
    .withColumn("material_id", F.upper(F.trim("material_id")),)
    .withColumn(
        "supplier_id",
        F.upper(
            F.trim("supplier_id")
...
# display(foundation_purchase_order_df)
...
merge_upsert(
    spark=spark,
    source_df=foundation_purchase_order_df,
