...
standardized_df = raw_df.withColumn('source_record_id', F.col('supplier_id'))
...
valid_records_df = standardized_df.filter(
    F.col('supplier_id').isNotNull() & F.col('source_system').isNotNull()
)
...
merge_upsert(
    spark=spark,
    source_df=foundation_supplier_df,
